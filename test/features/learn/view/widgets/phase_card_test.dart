...
void main() {
  group('PhaseCard', () {
    ...
    group('Option C stripe colour rule', () {
      // All six phases use phaseNOnLight for the 4px decorative stripe — the
      // alternative-hue token replaces every phase-colour foreground reference,
      // including the stripe, regardless of whether the original cleared 3:1.
      ...
      for (final id in [1, 2, 3, 4, 5, 6]) {
        testWidgets('phase $id stripe uses phase${id}OnLight', (tester) async {
          ...
          expect(
            ...
            reason:
                'phase $id stripe must use phaseNOnLight under Option C (alternative-hue rule)',
          );
        });
      }
    });
